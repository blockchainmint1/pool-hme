...
- [Stratum build & ZCU aux patch](mem://infra/stratum-build.md) — Two candidate source trees (LIVE-FINAL vs live-aux-issue-doge); parallel-make link race on algos/sha3 libs; always verify `stratum` mtime before install; `coind_create_job()` early-return patch fixes ZCU merge-mining (adapter returns -32601 for getblocktemplate).
