...
#### Correction #2 — it is NOT a stratum proxy, it is consumer-router NAT (2026-07-15 ~13:30 UTC)
...
`difficulty` row. Yiimp *is* tracking them individually — they are not
collapsed into one user. So the earlier "one TCP connection / one user"
framing was wrong. There are 976 distinct L9 → :3433 TCP sessions all
egressing through **one public IP via PAT** on the Conroe site's edge
device (reportedly a ~$35 TP-Link consumer router/firewall).

That box is the bottleneck. Three concurrent failure modes on cheap
consumer routers under this load:

1. **conntrack exhaustion** — consumer TP-Links cap at ~4–8k conntrack
   entries (often less). 976 long-lived stratum sessions + DNS/NTP/
   monitoring per unit puts you at/over the table. New connections drop,
   existing ones get evicted mid-share → reconnect storm → lost shares.
2. **PPS / CPU limit** — stratum is small packets (share submits + job
   notifies). A $35 router's SoC forwards maybe 20–50 kpps of NAT'd
   small packets. 1000 miners × several shares/sec × job-notify fanout
   is past that.
3. **Session churn** — dropped/evicted sessions force miner reconnects;
   in-flight shares are lost, vardiff resets, effective hashrate falls.

Net effect exactly matches the observed "~10% of expected hashpower
credited" — most work is generated by the ASICs but never reaches :3433.

#### Action items (revised)

1. **Replace the Conroe edge router.** MikroTik hEX / RB5009, Ubiquiti
   EdgeRouter / ER-X, or a small pfSense/OPNsense box. Target
   ≥ 32k conntrack, hardware or fastpath NAT. Confirm `conntrack -C`
   headroom after the swap. Cost is well under a day of lost mining.
2. **Split usernames per tank/container for diagnostics.** All L9s
   ...
   even though per-worker rows exist. Use worker-suffix convention:
   `ltc1q…worker.<container>-<tank>-<unit>` (Yiimp splits on `.`,
   payout stays on the wallet). "One login per 100" is a fine first
   step; per-unit is the eventual goal. Does not fix the router but
   makes the *next* incident diagnosable in minutes.
3. **Longer TCP keepalive on the miners** so reconnect churn drops when
   conntrack does evict.
4. **Cutover alignment (§8b).** Do the router swap and username split
   *before* re-homing Conroe to `stratum.pool.texitcoin.org:3433`,
   otherwise the cutover will look like it broke things when the
   underlying issue is still Conroe's LAN.
5. **ZCU `getblocktemplate` is broken** (`Zero Chill Units error
